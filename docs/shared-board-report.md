...
> 状态：已实现并真机验收入口 UI，328 个共享板断言全绿（2026-08-26）；`getBoardReport` 云函数是否已部署仍待用户确认
...
- **入口**：单板门面（`shared-board`）顶部工具栏左侧的中性胶囊「追番小结」，下钻到 `board-report`。与右侧视图切换按钮同排但语义、底色分开；归档态也可看，不设 phase 限制。
