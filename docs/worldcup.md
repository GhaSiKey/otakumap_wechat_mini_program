...
| 战绩页(simbet) | 顶部 tab 可切换 | 不做，移除战绩 tab | 战绩页重度依赖图表（累计盈亏曲线等），小程序需换 canvas 方案，成本高；本项目仅复刻赔率页 |
| 冠军收官条 | 闭幕时头部 champBanner | 已复刻（`meta.tournament`） | Web 版 7/20 闭幕后新增，小程序同步该块（不含战绩页） |
...
### 冠军收官条（赛事闭幕才显示）
- 数据源 `meta.tournament`，仅 `status === 'finished'` 时渲染，否则不出现
- 位置：页头金线之下、日历之上
- 内容：headline 播报 + 👑 冠军名（金色渐变字）+ 四强路径（🥇🥈🥉4️⃣，冠军名次高亮）+ 决赛/季军赛比分备注
- 预处理在 `transform.js` 的 `buildTournament`：名次+奖牌抽成 `PODIUM_RANKS` 配置，备注由 finalNote/thirdNote 拼接；缺失名次自动过滤
- 复用现有金色 CSS 变量（`--gold*`）与 `trophyGlow` 动画，固定深色，无需适配亮色

### 列表
- 按日期分组，每组头显示「日期 周几 + 阶段徽章 + 场次数 + 今天标记」
- 已完赛比赛（`result.status==='finished'`）显示全场/半场比分 + ✓ 完场徽章，卡片降透明度
