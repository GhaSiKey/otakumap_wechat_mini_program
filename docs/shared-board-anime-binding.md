...
> 状态：加番搜索带回已上线（2026-07-27）；2026-08-26 增加详情 `medium` 封面落库与存量 `small` 静默升级。
> 前置：弹弹play API 接入已完成（`animeMeta` 云函数 + 独立搜索/详情验收页，commit fbaf3db）
> 本文档描述如何把「弹弹play 番剧数据」接入共享追番板的加番与卡片流程。
>
...
| 决策 | 结论 |
|---|---|
| 自动刷新 | 业务元数据不周期刷新；仅旧 `small` 封面在进板时后台限量升级一次，不改活跃时间/历史 |
| 覆盖策略 | 业务字段只填空；封面仅把“同 sourceId 的可信弹弹play small”CAS 升级为详情 medium，其他 URL 不覆盖 |
...
| 字段 | 类型 | 说明 |
|---|---|---|
| `sourceId` | number \| null | 弹弹play animeId。标记「这张卡绑过弹弹play」，也是刷新入口的钥匙。非主键、可空、来源无关 |

复用已有字段承接弹弹play 数据：`cover`（封面 https 外链）、`totalEp`（正片集数）、`airStatus`（放送状态）。

### 2.2 封面清晰度策略（2026-08-26）

- `/api/v2/search/anime` 的 `imageUrl` 通常为 `small`（实测高约 200px），只负责搜索列表快速预览。
- `/api/v2/bangumi/{id}` 的 `imageUrl` 通常为不同 hash 的 `medium`（实测高约 400px），作为共享板最终展示与落库封面。
- 加番选择后本来就会补拉详情；详情成功时用其完整 `cover` 覆盖暂存的搜索小图，失败/超时仍以 `small` 降级完成加番。
- **禁止**把 URL 中的 `/small/` 替换成 `/medium/`：两档文件名 hash 不同，拼接路径会 404。
- 已落库的可信 `small`：板页首屏正常渲染后，每个页面实例静默调用一次 `animeMeta.upgradeBoardCovers`。服务端成员鉴权、同板 60 秒跨实例冷却、单次最多 8 条、复用详情缓存、按旧 URL 做条件更新；只改 `cover`，不 bump item/board `updateTime`、不写事件。
- 进度日志：开发者工具控制台过滤 `[shared-board][cover-upgrade]`；云开发日志过滤 `[animeMeta][cover-upgrade]`。可看到检测数、候选数、批次进度、升级/跳过/失败/剩余数与冷却状态。
...
- 回带字段：`{ sourceId, name, cover, totalEp }` —— 搜索页本身仍不含 airStatus。回到加番弹层后会后台补拉详情，把 `medium cover`、`airDay`、`isOnAir` 一并暂存；提交前在限定时间内等这次补拉，超时则按搜索数据降级。
...
2. **加番场景「带出不提交」**：搜番剧回带先预填番名/总集数和 `small` 预览（封面/sourceId 暂存 `_pickedMeta`）；详情回来后预览无缝切换为 `medium`。用户仍可改名/改集数，点「添加」才随 `addItem` 一并提交。选中 ≠ 提交。
...
- 封面策略测试：详情 `medium` 覆盖搜索 `small`、详情缺图降级、同 sourceId 重选竞态、可信 URL/sourceId 匹配、存量升级 CAS 只写 `cover`、同页面只触发一次。
...
**弹弹play API 接入遗留（若上一轮未部署，一并处理）**：
- `animeMeta` 云函数：重新部署（详情 medium 落库 + `upgradeBoardCovers` 存量升级）。普通云函数默认只有 3 秒，会触发 `FUNCTIONS_TIME_LIMIT_EXCEEDED`；必须到「云开发 → 云函数 → animeMeta → 函数配置（或高级设置）」把超时时间改为 **40 秒**，覆盖最坏 8 条分批补拉。
...
5. ✅ 加番搜索带回已真机验证通过（选中回填番名/集数/封面预览）
6. ✅ 补绑功能下架（见 §9）——存量老卡数据补完，不再需要
7. ⏳ **待用户**：在开发者工具删除云端已部署的 `bindItemMeta` 云函数（本地代码已删，云端残留不影响功能，清掉更干净）
8. ✅ 本地实现并测试：详情 `medium` 覆盖搜索 `small`，存量封面按板鉴权、限量、CAS 静默升级；⏳ 待重新部署 `animeMeta`（2026-08-26）
