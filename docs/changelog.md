...
## [2026-09-01] v1.6.0 - 圣地巡礼

### 新增

- **巡礼作品搜索**：复用弹弹play作品搜索，详情服务端从可信 Bangumi 页面 URL 解析 Subject ID；不调用 Bangumi API，不把弹弹play自有 `bangumiId` 当作 Subject ID
- **番剧收藏与最近浏览**：成功打开的巡礼作品可收藏到本机，并记录最近浏览；缺失映射直接展示空态，不引入人工维护后台
- **地点地图与搜索**：接入 Anitabi 公开 `lite` / `points/detail`，支持地图/列表、地点关键词、集数筛选和 Marker 聚合
- **地点详情**：参考图、集数时间、来源署名/链接、微信地图导航、巡礼礼仪和同集地点切换
- **数据代理与缓存**：新增 `pilgrimageData` 云函数，严格限制上游路径，归一图片/集数/时间/坐标，使用 6 小时缓存和最多 7 天瞬时故障降级

### 优化

- **真机巡礼界面**：搜索结果前置并压缩收藏/最近浏览重复内容；地图搜索与视图切换改为双行窄屏布局，地点搜索即时生效，列表长标题支持两行
- **图片失败降级**：作品封面优先使用弹弹play图片并尝试 Anitabi 兜底；地点图失败显示明确占位，详情大图按高清图 → 缩略图降级，并区分“没有图片”与“暂时无法加载”
- **地点详情层级**：修复 TDesign 按钮图标错位，保留单一全宽导航主操作，同集地点提前展示，无图状态压缩首屏高度

### 说明

- Anitabi 公共地点详情可能只是完整地图子集，页面明确并列展示公开返回数与完整地图数
- Anitabi 数据采用 CC BY-NC-SA 4.0；商业化前需另行确认授权

## [2026-07-27] v1.5.0 - 弹play 番剧搜索接入加番流 + 补绑下架

把弹play 番剧数据接入共享板加番流程（搜番名一键带出封面/集数），并下架已完成使命的老卡片补绑功能。

### 新增 / 优化

- **加番搜索带回**：加番弹层「搜番剧库」→ 搜索页 pick 模式列表直选 → 回带番名/集数/封面预览。走 EventChannel 单层回写（来源页 `navigateTo(events)` ↔ 搜索页 `getOpenerEventChannel().emit`），选中只「带出」不提交，点「添加」才随 addItem 入库
- **加番弹层重构（区分两种语义）**：未选中显示搜索区（整块可点 + 右箭头示「会跳转」）；选中后转封面预览卡（缩略图 + 番名 + 重选，封面缺失/失败走首字色块兜底）。分隔线文案随状态走——未选中「或手动填写」、已选中「确认信息（可修改）」，消除「已选中还写手动填写」的语义矛盾
- **搜索页 pick 模式提示**：顶部「点选一部，自动带回…」提示条 + 列表右侧「选择」标签替代箭头，明示点击是「带回」而非「进详情」
- **卡片封面渲染修复**（既有 bug）：番卡过去只渲染首字色块、从不渲染真封面 `it.cover`，补 `<image>` + binderror 回退；`coverErrorIds` 标记失败免闪烁，只在 URL 变化时精准清

### 修复

- **wx:else 夹注释崩渲染层**：加番弹层重构时在 `wx:if` 与 `wx:else` 之间夹了注释节点，触发 `Framework inner error (FLOW_DEPTH)`。`wx:else` 必须紧跟 `wx:if` 节点，中间不能有任何节点（含注释）

### 下线

- **老卡片补绑功能下架**：存量老卡数据已手动补完，补绑（详情弹层「关联番剧信息」）不再需要。删 `bindItemMeta` 云函数（本地）、`cloud-api.bindItemMeta`、`onBindSearchTap`/`_onPickedForBind`、详情补绑入口 + `.anime-bind-entry` 样式、`ANIME_BIND_COPY` 补绑文案、`transform.mergeMetaFillEmpty` + 18 条测试。保留加番搜索链路、`sourceId` 字段。测试 127→109 断言全绿。云端已部署的 `bindItemMeta` 待用户在开发者工具手动删除
- **首页「番剧搜索（验收）」临时入口撤除**：弹play 搜索已正式接入加番流，验收入口撤下；anime-search（normal 模式）+ anime-detail 页面代码保留，未来做「番剧图鉴/独立检索」可复用
